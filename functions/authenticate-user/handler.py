...
def handle(event, context):
    try:
        ...
        if not is_valid or not password_input or not otp_input:
            return make_response(
                400,
                {
                    "status": "error",
                    "message": validation_msg or "Champs 'username', 'password' et 'otp' requis."
                }
            )

        # Authentification de l'utilisateur
        status_code, result = authenticate_user(username, password_input, otp_input, secret_key)
        return make_response(
            status_code,
            result
        )
