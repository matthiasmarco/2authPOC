...
def authenticate_password(username, password_input, secret_key):
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT password, gendate, mfa, mfa_updated_at, expired, disabled FROM cofrap.users WHERE username = %s;", (username,))
            row = cur.fetchone()

            if row is None:
                return 404, {"status": "error", "message": "Utilisateur introuvable."}

            encrypt_password, gendate, encrypted_mfa, mfa_date, expired, disabled = row
            ...
            decrypted_pwd = decrypt_secret(encrypt_password, secret_key)
            if decrypted_pwd != password_input:
                return 401, {"status": "error", "message": "Mot de passe incorrect."}

            # Mise à jour de la date de validation du mot de passe
            current_time = datetime.now(timezone.utc)
            cur.execute(
                "UPDATE cofrap.users SET last_password_ok_at = %s WHERE username = %s;", (current_time, username,))
            return 200, {
                "status": "password-ok",
                "message": "Mot de passe correct. Veuillez entrer le code 2FA."
            }
    conn.close()
